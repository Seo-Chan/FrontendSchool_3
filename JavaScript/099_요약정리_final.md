...
```js
<script src="test.js"></script> // 변수가 중복될 수 있음
```
...
- nullish 병합 연산자(??)
  ...
  let result5 = null;
  let result6 = result5 ?? 100; // null이면 뒤의 값을 넣어준다. null이 아니면 앞에값
  ```
...
- Number(숫자)
  ...
  - 메서드 :
    ...
    - num.toFixed() //원하는 자릿수만큼 출력 ex) (10.1).toFixed()
  - Number()
...
- String(문자열)
  ...
  - 메서드 :
    ...
    - str.includes()
    - str.slice()
    - str.splice() -> 업데이트 사항 : 배열에 있는 메서드. str에는 없음!!
    - str.split() '!'.repeat(99).split('!')하면 배열100개가 됨
    - str.substring()
    - str.substr()
    - str.toLowerCase()
    - str.toUpperCase()
    ...
    "hello".repeat(100);
    "0".repeat(100);
    "5".repeat(100).split("").map(Number); //콜백함수가 Number로 들어간거임
    ```
...
- Array(배열) : object
  ...
  - 메서드 :
    - length
    - forEach // 단순순회. 새로운 array를 만들지 않음
    - map // 새로운 array를 만듦
    - filter
    - push / pop - mutable // 원본 수정가능한
    - slice - immutable // 원본 수정불가능한 것
    - splice - mutable
    - reduce - immutable 누적하는것
    - join
    - indexOf
    - includes
    - find
...
- Set : object
  ...
  let a = new Set([1, 2, 3]);
  let b = new Set([3, 4, 5]);
  const 합집합 = new Set([...a, ...b]); // new Set안하면 3이 두번 들어감
  const 교집합 = [...a].filter((x) => b.has(x));
  const 차집합1 = [...a].filter((x) => !b.has(x)); //a-b
  const 차집합2 = [...b].filter((x) => !a.has(x)); //b-a
  ```
